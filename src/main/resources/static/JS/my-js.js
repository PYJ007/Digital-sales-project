...
function show(object) {
    //获取当前标签id
    const tags = document.getElementsByClassName(object.className);
    for(let i=0;i<tags.length;i++)
    {
        tags[i].classList.remove("active");
    }
    const lis = document.getElementsByClassName("main-context");
    for(let i=0;i<lis.length;i++) {
        lis[i].style.display = "none";
    }
    const lis_tag = document.getElementById(object.id+"main");
    lis_tag.style.display="block";
    const tags_tag = document.getElementById(object.id);
    tags_tag.classList.add("active");
}
